...
namespace ForestDefenseGame
{
	...
	/// </summary>
	public class FDGEnemy:MonoBehaviour
	{
		...
		void SetSpeed( float setValue )
		{
			...
			if (GetComponent<Animation>()) GetComponent<Animation>()[moveAnimation.name].speed = 1 + moveSpeed * 0.2f * speedMultiplier;
			if (GetComponent<Animator>()) GetComponent<Animator>().SetFloat("SpeedMultiplier", 1 + moveSpeed * 0.2f * speedMultiplier);
        }
